...
class shader_litcube :
    public opengl_shader
{
public:
    shader_litcube(std::string _name, const char* vertexPath, const char* fragmentPath)//一种shader配一个
    {
        name = _name;
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);//存入数据
		glBindVertexArray(VAO);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
		glEnableVertexAttribArray(0);
    }
    unsigned int VBO, VAO, EBO;//顶点缓冲对象,顶点数组对象
	float vertices[288] = {
    ...
    void shaderload() override {//顺带用vao vbo绘制方块
        use();
        ...
        for (int i = 0; i < Lightlist.LightVector.size(); i++) {
            ...
            setVec3("lightcolor", l1ght->color);
            //setMat4("projection", maincar->projection);
            //setMat4("view", maincar->view);
